...
vector2_angle_between :: proc "c" (a: Vector2, b: Vector2) -> f32 {
	a0 := vector_normalize0(a)
	b0 := vector_normalize0(b)
	return f32(acos(f32(vector_dot(a0, b0))))
}

vector3_angle_between :: proc "c" (a: Vector3, b: Vector3) -> f32 {
	a0 := vector_normalize0(a)
	b0 := vector_normalize0(b)
	return f32(acos(f32(vector_dot(a0, b0))))
}

// NOTE: andai: haven't learned generics yet sorry
vector_angle_between :: proc {vector2_angle_between, vector3_angle_between}
...
vector2_dot :: proc "contextless" (a, b: Vector2) -> f32 #no_bounds_check {
	c : f32 = 0.0
	for i in 0..<2 {
		c += a[i] * b[i]
	}
	return c // NOTE: patched this, no idea what odin was doing with an empty return / named return value
}

@(require_results)
// vector_dot :: proc "contextless" (a, b: $T/[$N]$E) -> (c: E) where IS_NUMERIC(E) #no_bounds_check {
vector3_dot :: proc "contextless" (a, b: Vector3) -> f32 #no_bounds_check {
	c : f32 = 0.0
	for i in 0..<3 {
		c += a[i] * b[i]
	}
...
vector_dot :: proc { vector2_dot, vector3_dot }

@(require_results)
// vector_normalize0 :: proc "contextless" (v: $T/[$N]$E) -> T where math	(E) {
vector2_normalize0 :: proc "contextless" (v: Vector2) -> Vector2 {
	m := vector_length(v)
...
@(require_results)
// vector_normalize0 :: proc "contextless" (v: $T/[$N]$E) -> T where math	(E) {
vector3_normalize0 :: proc "contextless" (v: Vector3) -> Vector3 {
	m := vector_length(v)
	return 0 if m == 0 else v/m
}

vector_normalize0 :: proc {vector2_normalize0, vector3_normalize0}

@(require_results)
vector2_length :: proc "contextless" (v: Vector2) -> f32 {
	return f32(math.sqrt(vector_dot(v, v)))
}
...
vector_length :: proc {vector2_length, vector3_length}


// end andai monkey patch
...
@(require_results, deprecated="Prefer vector_angle_between(v1, v2)")
Vector2Angle :: proc "c" (v1, v2: Vector2) -> f32 {
	return vector_angle_between(v1, v2)
}
